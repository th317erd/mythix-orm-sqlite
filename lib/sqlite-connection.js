...
class SQLiteConnection extends SQLConnectionBase {
  ...
  async start() {
    let options   = this.getOptions();
    let filename  = options.filename || ':memory:';
    let dbOptions = {
      enableForeignKeyConstraints: options.foreignConstraints !== false,
    };

    if (options.readOnly)
      dbOptions.readOnly = true;

    this.db = new DatabaseSync(filename, dbOptions);
  }
  ...
  ///     | `parameters` | `Array` or `Object` | `undefined` | Parameters to bind for the [node:sqlite](https://nodejs.org/api/sqlite.html) prepared statement |
  ///
  /// Return: any
  ...
  async query(sql, _options) {
    ...
    try {
      ...
      let parameters  = (Nife.isNotEmpty(options.parameters)) ? [].concat(options.parameters) : [];
  ...
  async transaction(callback, _options) {
    ...
    try {
      ...
      try {
        ...
        if (options.logger)
          options.logger.error(`${this.constructor.name}::transaction: Error while attempting to ROLLBACK: `, error2);
      }
  ...
  formatResultsResponse(sqlStatement, columns, result) {
    return {
      rows:    result.map((row) => row.map((value) => (value instanceof Uint8Array) ? Buffer.from(value) : value)),
