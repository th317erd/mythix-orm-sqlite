...
class SQLiteQueryGenerator extends SQLQueryGeneratorBase {
  ...
  // node:sqlite's bundled SQLite is not compiled with
  // SQLITE_ENABLE_UPDATE_DELETE_LIMIT, so ORDER BY and
  // LIMIT/OFFSET are not supported in DELETE statements.
  generateDeleteStatement(Model, modelsOrQueryEngine, options) {
    let result = super.generateDeleteStatement(Model, modelsOrQueryEngine, options);
    return result.replace(/\s+ORDER\s+BY\b.+$/i, '');
  }
